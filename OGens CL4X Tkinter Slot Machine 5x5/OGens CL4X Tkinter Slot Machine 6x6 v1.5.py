...
STORE_ITEMS = {
    ...
    "Free Spins Purchase": {"cost": 300, "description": "Trigger 5 free spins"}
}
...
def save_game(balance, jackpot, stats, extra_spins, credits):
    ...
    try:
        data = {"balance": balance, "jackpot": jackpot, "stats": stats, "extra_spins": extra_spins, "credits": credits}
...
def check_paylines(reels, bet):
    ...
    for i, line in enumerate(paylines):
        ...
        if all(s == ref_symbol or s == "💎" for s in symbols):
            ...
            payout += line_payout
            extra_credits += random.randint(1, 50)  # Award random credits (1-50) for each winning payline
...
class SlotMachineGUI:
    def __init__(self, root):
        ...
        self.root.geometry("700x550")  # Increased window size for 6x6 grid and store
        self.root.update()
        ...
        # Add hover effects
        for btn in [self.spin_button, self.quit_button, self.leaderboard_button, self.reset_button, self.bet_plus, self.bet_minus, self.buy_button]:
    ...
    def buy_item(self):
        ...
        if item_name == "Extra Spin":
            ...
            self.free_spins_mode(spin_reels())  # Trigger free spins mode with new reels
        self.credits_label.config(text=f"Credits: {self.credits}")
        self.status_label.config(text=f"Purchased {item_name}!", fg="blue")
        save_game(self.balance, self.jackpot, self.stats, self.extra_spins, self.credits)
    ...
    def finalize_spin(self):
        ...
        self.stats["spins"] += 1
        self.stats["total_bet"] += self.bet if self.extra_spins == 0 else 0
        ...
        if win_lines:
            ...
            status_text = f"JACKPOT! Won {jackpot_payout:.1f} coins!"
    ...
    def free_spins_mode(self, initial_reels):
        ...
        for spin in range(FREE_SPINS):
            ...
            self.credits += extra_credits  # Add credits earned during free spins
            self.stats["spins"] += 1
            if win_lines:
                self.stats["wins"] += 1
                self.stats["total_won"] += payout + extra_credits
