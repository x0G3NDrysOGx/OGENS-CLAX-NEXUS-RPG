...
# Payout table (converted to integers for clean coin values)
PAYOUTS = {
    "🍒": 2, "🔔": 4, "🍋": 3, "7️⃣": 8,
    "⭐": 6, "💎": 12, "🍉": 3, "🍊": 2
}
...
# Store items (costs adjusted to integers)
STORE_ITEMS = {
...
def check_paylines(reels, bet):
    ...
    for i, line in enumerate(paylines):
        ...
        if all(s == ref_symbol or s == "💎" for s in symbols):
            ...
            multiplier = multiplier // 2 if len(line) < 5 else multiplier  # Integer division for partial paylines
