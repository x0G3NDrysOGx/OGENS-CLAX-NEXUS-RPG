...
class SlotMachineGUI:
    def __init__(self, root):
        ...
        self.root.geometry("550x500")  # Adjusted window size for better readability
        self.root.update()
        ...
        # Reel labels (5x5 grid) - Unchanged font size for symbols
        ...
        # Balance, jackpot, and bet display - Increased font size to 8
        self.balance_label = tk.Label(root, text=f"Balance: {self.balance} coins", font=("Arial", 8), fg="white", bg="black")
        self.balance_label.grid(row=ROWS, column=0, columnspan=REELS, pady=1)

        self.jackpot_label = tk.Label(root, text=f"Jackpot: {self.jackpot:.1f} coins", font=("Arial", 8), fg="white", bg="black")
        self.jackpot_label.grid(row=ROWS+1, column=0, columnspan=REELS, pady=1)

        self.bet_label = tk.Label(root, text=f"Bet: {self.bet} coin(s)", font=("Arial", 8), fg="white", bg="black")
        self.bet_label.grid(row=ROWS+2, column=0, columnspan=REELS, pady=1)

        # Bet adjustment - Increased font size to 8
        tk.Label(root, text="Bet Amount:", font=("Arial", 8), fg="white", bg="black").grid(row=ROWS+3, column=0, pady=1)
        self.bet_entry = tk.Entry(root, width=4, font=("Arial", 8))
        self.bet_entry.insert(0, "1")
        self.bet_entry.grid(row=ROWS+3, column=1, pady=1)
        self.bet_plus = tk.Button(root, text="+", command=self.increase_bet, bg="green", fg="white", activebackground="lightgreen", font=("Arial", 8))
        self.bet_plus.grid(row=ROWS+3, column=2, sticky="w", padx=1)
        self.bet_minus = tk.Button(root, text="-", command=self.decrease_bet, bg="green", fg="white", activebackground="lightgreen", font=("Arial", 8))
        self.bet_minus.grid(row=ROWS+3, column=2, sticky="e", padx=1)

        # Stats display - Increased font size to 8
        self.stats_label = tk.Label(root, text=self.get_stats_text(), font=("Arial", 8), fg="white", bg="black", justify="left")
        self.stats_label.grid(row=0, column=REELS, rowspan=3, padx=3, sticky="n")

        # Payline display - Increased font size to 8
        self.payline_label = tk.Label(root, text="Paylines: None", font=("Arial", 8), fg="white", bg="black", justify="left")
        self.payline_label.grid(row=0, column=REELS+1, rowspan=3, padx=3, sticky="n")

        # Status display - Increased font size to 8
        self.status_label = tk.Label(root, text="Welcome! Press Spin to play!", font=("Arial", 8), fg="blue", bg="black")
        self.status_label.grid(row=ROWS+4, column=0, columnspan=REELS+2, pady=1)

        # Buttons - Increased font size to 8
        self.spin_button = tk.Button(root, text="Spin", command=self.spin, bg="red", fg="white", activebackground="pink", font=("Arial", 8))
        self.spin_button.grid(row=ROWS+5, column=0, columnspan=2, pady=1)
        self.quit_button = tk.Button(root, text="Quit", command=self.quit, bg="purple", fg="white", activebackground="violet", font=("Arial", 8))
        self.quit_button.grid(row=ROWS+5, column=2, pady=1)
        self.leaderboard_button = tk.Button(root, text="Leaderboard", command=self.show_leaderboard, bg="blue", fg="white", activebackground="lightblue", font=("Arial", 8))
        self.leaderboard_button.grid(row=ROWS+5, column=3, columnspan=1, pady=1)
        self.reset_button = tk.Button(root, text="Reset", command=self.reset_game_prompt, bg="orange", fg="white", activebackground="lightyellow", font=("Arial", 8))
