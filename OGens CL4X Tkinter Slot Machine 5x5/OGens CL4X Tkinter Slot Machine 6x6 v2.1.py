import secrets  # Changed from random to secrets for true randomness
import time
import json
import os
...
FREE_SPINS = 5
JACKPOT_BASE = 1000
JACKPOT_INCREMENT = 1  # Changed to integer increment
...
def spin_reels():
    ...
    return [[secrets.choice(SYMBOLS) for _ in range(REELS)] for _ in range(ROWS)]  # Changed to secrets.choice
...
def check_paylines(reels, bet):
    ...
    paylines = [
        ...
        [(0, 5), (1, 4), (2, 3), (3, 2), (4, 3), (5, 4)],
        [(0, 0), (1, 2), (2, 4), (3, 3), (4, 2), (5, 0)],
        [(0, 1), (1, 3), (2, 5), (3, 4), (4, 2), (5, 1)],
        [(0, 4), (1, 2), (2, 0), (3, 1), (4, 3), (5, 4)],
    ...
    for i, line in enumerate(paylines):
        ...
        if all(s == ref_symbol or s == "💎" for s in symbols):
            ...
            payout += line_payout
            extra_credits += secrets.randbelow(100) + 1  # Changed to secrets.randbelow for true randomness (1-100)
...
def check_jackpot(bet, jackpot):
    ...
    if 1 <= bet <= 100 and secrets.randbelow(1000) == 0:  # Changed to secrets.randbelow for true randomness
        return jackpot
    return 0
...
class SlotMachineGUI:
    def __init__(self, root):
        ...
        self.jackpot_label = tk.Label(root, text=f"Jackpot: {int(self.jackpot)} coins", font=("Arial", 7, "bold"), fg="white", bg="black")
    ...
    def reset_game_prompt(self):
        ...
        if messagebox.askyesno("Reset Game", "Are you sure you want to reset the game? This will delete all progress!"):
            ...
            self.jackpot_label.config(text=f"Jackpot: {int(self.jackpot)} coins")
    ...
    def finalize_spin(self):
        ...
        if win_lines:
            ...
            self.jackpot = JACKPOT_BASE  # Reset to integer base
            status_text = f"JACKPOT! Won {int(jackpot_payout)} coins!"
    ...
    def spin(self):
        ...
        try:
            ...
            if self.extra_spins > 0:
                ...
                self.jackpot += int(self.bet * JACKPOT_INCREMENT)  # Convert to integer
            self.spinning = True
            self.animate_spin()
        except ValueError:
