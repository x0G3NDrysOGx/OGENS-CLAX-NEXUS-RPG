...
def inventory():
    ...
    while True:
        ...
        print(Fore.GREEN + "NEON GRID RUNNER - D5+D10" + Style.RESET_ALL)
        print(Fore.GREEN + "--------------------------------------" + Style.RESET_ALL)
...
def main():
    """Main game loop."""
    global player, current_companion, stat_changes, quantum_core_active, deactivated_companions, last_path_chosen
    ...
    print(Fore.GREEN + "\n\n        Press Enter to jack in" + Style.RESET_ALL)
    ...
    while True:
        ...
        if choice in ["1", "2", "3", "4"]:
            ...
            player["crypto_chips"] += crypto_change
            drive_change = changes.get("data_drive", 0)
            player["data_drive"] += drive_change
            core_change = changes.get("quantum_core", 0)
            player["quantum_core"] += core_change
            patch_change = changes.get("nano_patch", 0)
            player["nano_patch"] += patch_change
            key_change = changes.get("neon_key", 0)
            player["neon_key"] += key_change
            ...
            if was_deactivated and is_positive_outcome(changes):
                stat_changes_list.append(Fore.GREEN + f"{current_companion['name']} revived with +14 health" + Style.RESET_ALL)
