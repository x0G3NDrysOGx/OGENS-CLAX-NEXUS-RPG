...
# List of cyberpunk-themed ASCII art designs
ascii_art_list = [
    # Start screen (neon circuit board with "NET")
    """
   |===>|
   | NET |
   | *** |
   ...
   |===>|
""",
    # Path 1 (Data Vaults - server rack with blinking lights)
    """
   |||||
   | * * |
   | *** |
   ...
   |||||
""",
    # Path 2 (Neon Slums - flickering neon sign)
    """
   ~~~~~
   | *  |
   | *** |
   ...
   ~~~~~
""",
    # Path 3 (Corporate Spire - skyscraper with data streams)
    """
   /| |\\
   /|*|*\\
   /|***|\\
   ...
   /| |\\
""",
    # Path 4 (Darknet Grid - virtual grid with pulsing nodes)
    """
   ------
   | *  *|
   |*  * |
   ...
   ------
""",
    # Inventory (holographic interface with item slots)
    """
   +++++
   + *** +
   +*   *+
   ...
   +++++
""",
    # Win/Loss (digital orb with "WIN" or "FLATLINE")
    """
   oooo
   o *** o
   o*WIN*o
...
# Function to get random ASCII art, ensuring no consecutive repetition
...
def main():
    ...
    print(Fore.GREEN + "--------------------------------------" + Style.RESET_ALL)
    print(Fore.GREEN + get_random_ascii() + Style.RESET_ALL)
    print(Fore.GREEN + "OGENS NEON NEXUS" + Style.RESET_ALL)
    print(Fore.GREEN + "SURVIVE THE UNDERWORLD" + Style.RESET_ALL)
    ...
    while True:
        ...
        if choice in ["1", "2", "3", "4"]:
            path = int(choice)
            path_messages = {
                1: "INFILTRATING THE DATA VAULTS...",
                2: "NAVIGATING THE NEON SLUMS...",
                3: "ASCENDING THE CORPORATE SPIRE...",
                4: "HACKING INTO THE DARKNET GRID..."
            }
            os.system("cls")
            print(Fore.GREEN + "--------------------------------------" + Style.RESET_ALL)
            print(Fore.GREEN + "OGENS NEON NEXUS - D5+D5" + Style.RESET_ALL)
            ...
            with open("GameLog.txt", "a") as f:
                f.write(f"{now.strftime('%m/%d/%Y %H:%M')} - Quit - Score: {player['score']}, Health: {player['health']}, Data Cores: {player['artifacts']}, Quantum Mods: {player['quantum_mod']}, Level: {player['level']}\n")
        ...
        if player["health"] <= 0:
            ...
            print(Fore.GREEN + f"Time: {now.strftime('%H:%M')}" + Style.RESET_ALL)
            with open("GameLog.txt", "a") as f:
                f.write(f"{now.strftime('%m/%d/%Y %H:%M')} - Loss - Score: {player['score']}, Health: {player['health']}, Data Cores: {player['artifacts']}, Quantum Mods: {player['quantum_mod']}, Level: {player['level']}\n")
            print(Fore.GREEN + "\nPress Enter to end..." + Style.RESET_ALL)
            input()
            break
        if player["score"] >= 6000 and player["artifacts"] >= 300 and player["quantum_mod"] >= 50:
            ...
            print(Fore.GREEN + f"Time: {now.strftime('%H:%M')}" + Style.RESET_ALL)
            with open("GameLog.txt", "a") as f:
                f.write(f"{now.strftime('%m/%d/%Y %H:%M')} - Win - Score: {player['score']}, Health: {player['health']}, Data Cores: {player['artifacts']}, Quantum Mods: {player['quantum_mod']}, Level: {player['level']}\n")
            print(Fore.GREEN + "\nPress Enter to end..." + Style.RESET_ALL)
            input()
            break
