...
if defined character if "!character!" neq "" (
    if not defined %character%_health (
        call :set_random_stats !character!
    )
)
...
rem Path 1: Left Tunnel (Grid: 1-6, 3 positive, 3 negative)
if "!choice!"=="1" (
    ...
    if !adjusted_grid! == 3 (set "result=Poisonous gas seeps from the walls" & set /a comp_health_change=-15 + !char_strength! & set /a xp_change=2 & set /a player_health_change=-9 & set /a display_player_health_change=!player_health_change! & set show_health_change=1)
...
rem Path 2: Right Tunnel (Grid: 1-6, 3 positive, 3 negative)
if "!choice!"=="2" (
    if !adjusted_grid! == 1 (set "result=Slippery rocks cause a fall" & set /a comp_health_change=-15 + !char_strength! & set /a xp_change=2 & set /a player_health_change=-9 & set /a display_player_health_change=!player_health_change! & set show_health_change=1)
    ...
    if !adjusted_grid! == 3 (set "result=Thorny tendrils lash out" & set /a comp_health_change=-15 + !char_strength! & set /a xp_change=2 & set /a player_health_change=-9 & set /a display_player_health_change=!player_health_change! & set show_health_change=1)
    if !adjusted_grid! == 4 (set "result=An urn discloses a shimmering relic" & set /a score_change=2 + !char_strength! + !level! & set /a relic_change=1 & set /a xp_change=4 & set /a comp_health_change=2 & set /a player_health_change=0 & set /a display_player_health_change=!player_health_change! & set show_health_change=1)
...
rem Path 3: Hidden Stairs (Grid: 1-6, 3 positive, 3 negative)
if "!choice!"=="3" (
    ...
    if !adjusted_grid! == 3 (set "result=A swarm of bats overwhelms" & set /a comp_health_change=-15 + !char_strength! & set /a xp_change=2 & set /a player_health_change=-9 & set /a display_player_health_change=!player_health_change! & set show_health_change=1)
    if !adjusted_grid! == 4 (set "result=Falling pebbles hint at a secret" & set /a score_change=2 + !char_strength! + !level! & set /a xp_change=4 & set /a comp_health_change=2 & set /a player_health_change=0 & set /a display_player_health_change=!player_health_change! & set show_health_change=1)
...
rem Path 4: Crystal Cavern (Grid: 1-6, 3 positive, 3 negative)
if "!choice!"=="4" (
    ...
    if !adjusted_grid! == 3 (set "result=A crystal hawk dives" & set /a comp_health_change=-15 + !char_strength! & set /a xp_change=2 & set /a player_health_change=-9 & set /a display_player_health_change=!player_health_change! & set show_health_change=1)
    if !adjusted_grid! == 4 (set "result=A shimmering relic pulses with power" & set /a score_change=3 + !char_strength! + !level! & set /a relic_change=1 & set /a xp_change=4 & set /a comp_health_change=2 & set /a player_health_change=0 & set /a display_player_health_change=!player_health_change! & set show_health_change=1)
...
if exist SaveGame.txt (
    copy SaveGame.txt SaveGame.bak >nul 2>nul || (
...
(
    ...
    echo Failed to write save file...
    goto save_failed
)
rem Verify the save file
if exist SaveGame.txt (
    for %%F in (SaveGame.txt) do if %%~zF gtr 0 (
        findstr /c:"score=" /c:"health=" /c:"max_health=" /c:"artifacts=" /c:"level=" SaveGame.txt >nul 2>nul
        if not errorlevel 1 (
            set "save_success=1"
        )
    )
...
if !save_success! equ 0 (
    if !retry_count! lss 2 (
        ...
        if exist SaveGame.bak (
            copy SaveGame.bak SaveGame.txt >nul 2>nul
