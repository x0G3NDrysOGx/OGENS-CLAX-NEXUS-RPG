...
def roll_dice():
    ...
    return die1, die2, outcome, outcome  # No luck modifier, adjusted_outcome = outcome
...
def path_outcomes(path, outcome):
    ...
    result, changes = outcomes.get(path, {}).get(outcome, ("No outcome defined", {}))
...
def main():
    ...
    # Select random companion at start (include all companions)
    ...
    while True:
        ...
        if choice in ["1", "2", "3", "4"]:
            ...
            # Select random companion (include all companions)
            companion_name = secrets.choice(companions)
            current_companion.update({
                "name": companion_name,
                "health": max(0, min(companion_stats[companion_name]["health"], 100)),
