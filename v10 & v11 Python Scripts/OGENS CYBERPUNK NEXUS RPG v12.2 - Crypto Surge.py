...
def main():
    """Main game loop."""
    global player, current_companion, stat_changes, quantum_core_active, deactivated_companions, companion_stats
    ...
    # Assign a new random companion at start, ensuring it's not deactivated
    available_companions = [name for name in companions if name not in deactivated_companions]
    if not available_companions:  # If all companions are deactivated, reset them
        deactivated_companions.clear()
        available_companions = companions.copy()
        stat_changes = "All companions were deactivated; resetting companion list."
        print(Fore.RED + "Warning: All companions were deactivated. Resetting companion list." + Style.RESET_ALL)
    ...
    while True:
        ...
        if choice == "0":
            ...
            try:
                ...
                # Select a new random companion for each path event, including deactivated ones
                available_companions = companions.copy()  # Use all companions
                if not available_companions:  # Fallback if companions list is empty
